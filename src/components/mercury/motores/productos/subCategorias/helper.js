import motor1 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/Imagen 312.png"
import motor2 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/Imagen 311.png"
import motor3m from "../../../../../assets/mercury/motores/cuatroTempos/detalles/3.5-m-4s.png"
import m54s from "../../../../../assets/mercury/motores/cuatroTempos/detalles/5m-ml-4s.png"
import m994s from "../../../../../assets/mercury/motores/cuatroTempos/detalles/9.9-m-ml-4s.png"
import m15ml from "../../../../../assets/mercury/motores/cuatroTempos/detalles/15-m-ml-4s.png"
import el1545 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/15-e-el-4s.png"
import elpt40 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/40-elpt-4s-efi.png"
import ml20 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/20-m-ml-4s.png"
import elpt50 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/50-elpt-4s-efi.png"
import elpt60 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/60-elpt-4s-efi-ct.png"
import cuatroTempos75elpt from "../../../../../assets/mercury/motores/cuatroTempos/detalles/75-elpt-4s-efi.png"
import cuatroTempos90elptefi from "../../../../../assets/mercury/motores/cuatroTempos/detalles/90-elpt-4s-efi.png"
import cuatroTempos115elpt from "../../../../../assets/mercury/motores/cuatroTempos/detalles/115-elpt-4s-efi.png"
import cuatroTempos115elptct from "../../../../../assets/mercury/motores/cuatroTempos/detalles/115-elpt-4s-efi-ct.png"
import cuatroTempos115elptpro from "../../../../../assets/mercury/motores/cuatroTempos/detalles/115-elpt-4s-efi-pro-xs.png"
import cuatroTempos150xl from "../../../../../assets/mercury/motores/cuatroTempos/detalles/150-l-xl-4s-efi.png"
import cuatroTempos150xl4sefipro from "../../../../../assets/mercury/motores/cuatroTempos/detalles/150-l-xl-4s-efi-pro-xs.png"
import cuatroTempos200 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/PB 200 V6 CMS R3_4 Port_matteBlack.png"
import cuatroTempos250 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/PB 250 V8 CMS R3_4 Port_matteBlack.png"
import cuatroTempos300 from "../../../../../assets/mercury/motores/cuatroTempos/detalles/300hp_V8_PB_CMS_FS_Port_Rear_3-4.png"

import seapro from "../../../../../assets/mercury/motores/seaPro/detalles/Imagen 314.png"
import seapro60elpt from "../../../../../assets/mercury/motores/seaPro/detalles/60-elpt-4s-sp.png"
import seapro75 from "../../../../../assets/mercury/motores/seaPro/detalles/75-elpt-4s-sp.png"
import seapro150sp from "../../../../../assets/mercury/motores/seaPro/detalles/150-l-xl-4s-sp.png"
import seapro115 from "../../../../../assets/mercury/motores/seaPro/detalles/115-elpt-4s-sp.png"
import seapro90 from "../../../../../assets/mercury/motores/seaPro/detalles/90-elpt-4s-sp.png"
import seapro1 from "../../../../../assets/mercury/motores/seaPro/detalles/Imagen 313.png"
import seapro2 from "../../../../../assets/mercury/motores/seaPro/detalles/Imagen -1.png"
import seapro200 from "../../../../../assets/mercury/motores/seaPro/detalles/200hp_SP_FS_V6_XL_RP3-4.png"
import seapro500 from "../../../../../assets/mercury/motores/seaPro/detalles/V12_500HP_SeaPro_Rear_3-4_PORT.png"
import verado from "../../../../../assets/mercury/motores/verado/detalle/verado.png"
import verado2 from "../../../../../assets/mercury/motores/verado/detalle/NoPath - copia.png"

import verado250hp from "../../../../../assets/mercury/motores/verado/detalle/250hp_V8_PB_AMS_FS_Port_Rear_3-4.png"
import verado300hp from "../../../../../assets/mercury/motores/verado/detalle/300hp_V8_PB_AMS_FS_Port_Rear_3-4.png"
import veradoV12_600 from "../../../../../assets/mercury/motores/verado/detalle/V12_600HP_PB_Rear_3-4_PORT.png"
import verado10_port_b_400 from "../../../../../assets/mercury/motores/verado/detalle/V10-Verado_Rear_3_4_Port_B_400.png"
import verado10_port_b_350 from "../../../../../assets/mercury/motores/verado/detalle/V10-Verado_Rear_3_4_Port_B_350.png"

// import dosTempos from "../../../../../assets/mercury/motores/dosTempos/detalles/NoPath - copia (3).png"
import dosTempos33m from "../../../../../assets/mercury/motores/dosTempos/detalles/3.3-m.png"
import dosTempos5mml from "../../../../../assets/mercury/motores/dosTempos/detalles/5-m-ml.png"
import dosTempos15mSuper from "../../../../../assets/mercury/motores/dosTempos/detalles/15-m-super.png"
import dosTempos25ml from "../../../../../assets/mercury/motores/dosTempos/detalles/25-m-ml-sea-pro.png"
import dosTempos30ml from "../../../../../assets/mercury/motores/dosTempos/detalles/30-m-ml-super.png"
import dosTempos40ml from "../../../../../assets/mercury/motores/dosTempos/detalles/40-m-ml-super.png"
import dosTempos40eoelo from "../../../../../assets/mercury/motores/dosTempos/detalles/40-eo-elo-super.png"
import dosTempos40elpto from "../../../../../assets/mercury/motores/dosTempos/detalles/40-elpto.png"
import dosTempos50elpto from "../../../../../assets/mercury/motores/dosTempos/detalles/50-elpto.png"
import dosTempos2 from "../../../../../assets/mercury/motores/dosTempos/detalles/NoPath - copia (4).png"

import racing from "../../../../../assets/mercury/motores/racing/detalles/NoPath - copia (10).png"

import fichaTresPuntoCinco from "../../../../../assets/mercury/motores/cuatroTempos/fichas/3.5-m-ml-4s.pdf"
import fichaCincom from "../../../../../assets/mercury/motores/cuatroTempos/fichas/5m-ml-4s.pdf"
import fichaNuevePuntoNueve from "../../../../../assets/mercury/motores/cuatroTempos/fichas/9.9-m-ml-4s.pdf"
import fichaQuinceE from "../../../../../assets/mercury/motores/cuatroTempos/fichas/15-e-el-4s.pdf"
import fichaQuinceM from "../../../../../assets/mercury/motores/cuatroTempos/fichas/15-m-ml-4s.pdf"
import fichaVeinteM from "../../../../../assets/mercury/motores/cuatroTempos/fichas/20-m-ml-4s.pdf"
import fichaCuarentaEl from "../../../../../assets/mercury/motores/cuatroTempos/fichas/40-elpt-4s-efi.pdf"
import fichaCincuentaElpt from "../../../../../assets/mercury/motores/cuatroTempos/fichas/50-elpt-4s-efi.pdf"
import fichaSesentaCt from "../../../../../assets/mercury/motores/cuatroTempos/fichas/60-elpt-4s-efi-ct.pdf"
import fichaSesenta from "../../../../../assets/mercury/motores/cuatroTempos/fichas/60-elpt-4s-efi.pdf"

import ficha_cuatroTiempos_Setenta from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/75-elpt-4s-efi.pdf"
import ficha_cuatroTiempos_Noventaelpt from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/90-elpt-4s-efi.pdf"
import ficha_cuatroTiempos_CientoQuinceCt from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/115-elpt-4s-efi-ct.pdf"
import ficha_cuatroTiempos_CientoQuince_efiproxs from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/115-elpt-4s-efi-pro-xs.pdf"
import ficha_cuatroTiempos_CientoQuince_elpt_s from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/115-elpt-4s-efi.pdf"
import ficha_cuatroTiempos_CientoCincuenta_lx_efi from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/150-l-lx-4s-efi.pdf"
import ficha_cuatroTiempos_CientoCincuenta_xl_efi_pro from "../../../../../assets/mercury/motores/cuatroTempos/fichas/75-150 hp/150-l-xl-4s-efi-pro-xs.pdf"
import ficha_cuatroTiempos_DoscientosCincuenta from "../../../../../assets/mercury/motores/cuatroTempos/fichas/200-300 hp/250-l-lx-4s-efi.pdf"
import ficha_cuatroTiempos_Tresientos from "../../../../../assets/mercury/motores/cuatroTempos/fichas/200-300 hp/300-l-xl-4s-efi.pdf"
import ficha_cuatroTiempos_Doscientos from "../../../../../assets/mercury/motores/cuatroTempos/fichas/200-300 hp/Diptico-Mercury-V6-200hp.pdf"


import ficha_seapro_sesenta_elpcuatro from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/60-elpt-4s-sp (1).pdf"
import ficha_seapro_sesenta_elptcuatros from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/60-elpt-4s-sp.pdf"
import ficha_seapro_setentaCinco from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/75-elpt-4s-sp (1).pdf"
import ficha_seapro_setentaCincoCopia from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/75-elpt-4s-sp.pdf"
import ficha_seapro_NoventaElpt from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/90-elpt-4s-sp (1).pdf"
import ficha_seapro_Noventa_elpt from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/90-elpt-4s-sp.pdf"
import ficha_seapro_cientoQuince_elpt from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/115-elpt-4s-sp (1).pdf"
import ficha_seapro_ciento_quince from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/115-elpt-4s-sp.pdf"
import ficha_seapro_ciento_cincuenta from "../../../../../assets/mercury/motores/seaPro/fichas/60 -150 hp/150-l-xl-4s-sp.pdf"
import ficha_seapro_doscientos_unico from "../../../../../assets/mercury/motores/seaPro/fichas/200 hp/200-l-lx-4s-sp.pdf"
import ficha_seapro_quinientos_unico from "../../../../../assets/mercury/motores/seaPro/fichas/500 hp/seapro-500.pdf"

import ficha_verado_doscincuenta from "../../../../../assets/mercury/motores/verado/fichas/200-300 hp/250-hp.pdf"
import ficha_verado_trescientos from "../../../../../assets/mercury/motores/verado/fichas/200-300 hp/300-hp.pdf"

import ficha_verado_seiscientos from "../../../../../assets/mercury/motores/verado/fichas/600/verado-600.pdf"
import ficha_verado_tresCincuenta_and_cuatrocientos from "../../../../../assets/mercury/motores/verado/fichas/350/V10_Verado_350 Y 400 HP.pdf"

import ficha_dosTiempos_tres_tres from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/3.3-m.pdf"
import ficha_dosTiempos_cinco from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/5-m-ml.pdf"
import ficha_dosTiempos_quince from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/15-m-super.pdf"
import ficha_dosTiempos_veinticinco from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/25-m-ml-sea-pro.pdf"
import ficha_dosTiempos_treinta from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/30-m-ml-super.pdf"
import ficha_dosTiempos_cuarenta_elpto from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/40-elpto.pdf"
import ficha_dosTiempos_cuarenta_elo_super from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/40-eo-elo-super.pdf"
import ficha_dosTiempos_cuarenta_ml_super from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/40-m-ml-super.pdf"
import ficha_dosTiempos_cincuenta_elpto from "../../../../../assets/mercury/motores/dosTempos/fichas/3.3 90/50-elpto.pdf"

import ficha_racing_cuatrocientoscincuenta from "../../../../../assets/mercury/motores/racing/fichas/450/racing-450hp.pdf"



export const cuatroTemposInfo = [
    // [

    //     {
    //         id: "3.5 M 4S",
    //         nombre: "3.5 M 4S",
    //         caracteristicas: `hp / kW: 3.5 / 2.6\n Tipo de motor: 1 cilindro\nDesplazamiento (CID/ CC): 	5.2 / 85\nEmpuje total (RPM): 5000-6000\nLongitud de pata: 15" / 381 mm | 20" / 508 mm\nProporción de la caja de engranajes: 2.15:1`,
    //         imagen: motor1,
    //         imagenDimension: "80%",
    //         slides: 3,
    //         pdf: fichaTresPuntoCinco
    //     },
    //     {
    //         id: "5M/ ML 4S",
    //         nombre: "5 M 4S",
    //         caracteristicas: `hp / kW:5/3.7\n
    //     Tipo de motor: 1 cilindro\n
    //     Desplazamiento (CID/ CC): 	7.5 / 123\n 
    //     Empuje total (RPM): 4500 - 5500\n
    //     Longitud de pata: 15" / 381mm | 25" / 635mm\n
    //     Proporción de la caja de engranajes: 2.15:1\n
    //     `,
    //         imagen: motor2,
    //         imagenDimension: "80%",
    //         pdf: fichaCincom
    //     },
    //     {
    //         id: "9.9 M/ ML 4S",
    //         nombre: "9.9 M/ ML 4S",
    //         caracteristicas: `hp / kW:5/3.7\n
    //     Tipo de motor: 1 cilindro\n
    //     Desplazamiento (CID/ CC): 	7.5 / 123\n 
    //     Empuje total (RPM): 4500 - 5500\n
    //     Longitud de pata: 15" / 381mm | 25" / 635mm\n
    //     Proporción de la caja de engranajes: 2.15:1\n
    //     `,
    //         imagen: motor2,
    //         imagenDimension: "80%",
    //         pdf: fichaNuevePuntoNueve
    //     },
    //     {
    //         id: "15 M/ ML 4S",
    //         nombre: "15 M/ ML 4S",
    //         caracteristicas: `hp / kW:5/3.7\n
    //     Tipo de motor: 1 cilindro\n
    //     Desplazamiento (CID/ CC): 	7.5 / 123\n 
    //     Empuje total (RPM): 4500 - 5500\n
    //     Longitud de pata: 15" / 381mm | 25" / 635mm\n
    //     Proporción de la caja de engranajes: 2.15:1\n
    //     `,
    //         imagen: motor2,
    //         imagenDimension: "80%",
    //         pdf: fichaQuinceM
    //     },
    // ],
    [
        [

            {

                id: "3.5 M 4S",
                nombre: "3.5 M 4S",
                caracteristicas: `hp / kW	3.5 / 2.6\n
            Tipo de motor	1 cilindro\n
            Desplazamiento (CID/ CC)	5.2 / 85\n
            Empuje total (RPM)	5000-6000\n
            Longitud de pata	15" / 381 mm | 20" / 508 mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	41 lbs / 18 kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	CDI c/avance de encendido electrónico\n
            Sistema de combustible	Carburado\n
            Sistema de escape	Soporte superior\n
            Diámetro y carrera	2.16 x 1.42" / 55 x 36 mm\n
            `,
                imagen: motor3m,
                imagenDimension: "45%",
                pdf: fichaTresPuntoCinco,
                slides: 9

            },
            {

                id: "5M/ ML 4S",
                nombre: "5M/ ML 4S",
                caracteristicas: `hp / kW	5/3.7\n
            Tipo de motor	1 cilindro\n
            Desplazamiento (CID/ CC)	7.5 / 123\n
            Empuje total (RPM)	4500 - 5500\n
            Longitud de pata	15" / 381mm | 25" / 635mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	57lbs / 26kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	CDI c/ Avance de Encendido Electrónico\n
            Sistema de combustible	Carburado\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.32 x 1.77" / 59.45mm\n
            `,
                imagen: m54s,
                imagenDimension: "45%",
                pdf: fichaCincom

            },
            {

                id: "9.9 M/ ML 4S",
                nombre: "9.9 M/ ML 4S",
                caracteristicas: `hp / kW	9.9 / 7.3\n
            Tipo de motor	en línea 2\n
            Desplazamiento (CID/ CC)	2.8 / 208\n
            Empuje total (RPM)	5000 - 6000\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	2.08:1
            Peso en seco *el modelo más ligero disponible	84lbs / 38kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	CDI c/ Avance de Encendido Electrónico\n
            Sistema de combustible	Carburado\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.16 x 1.73" / 55 x 44 mm
            `,
                imagen: m994s,
                imagenDimension: "30%",
                pdf: fichaNuevePuntoNueve
            },
            {

                id: "15 M/ ML 4S",
                nombre: "15 M/ ML 4S",
                caracteristicas: `hp / kW	15 / 11.1\n
            Tipo de motor	en línea 2\n
            Desplazamiento (CID/ CC)	21.4 / 351\n
            Empuje total (RPM)	5000 - 6000\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	115lbs / 52kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	CDI c/ Avance de Encendido Electrónico\n
            Sistema de combustible	Carburado\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.40 x 2.36" / 61 x 60mm\n
            `,
                imagen: m15ml,
                imagenDimension: "32%",
                pdf: fichaQuinceM
            },
            {

                id: "15 E/EL 4S",
                nombre: "15 E/EL 4S",
                caracteristicas: `hp / kW	15 / 11.1\n
            Tipo de motor	en línea 2\n
            Desplazamiento (CID/ CC)	21.4 / 351\n
            Empuje total (RPM)	5000 - 6000\n
            Longitud de pata	15" / 381mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	115lbs / 52kg\n
            Ignición	CDI c/ Avance de Encendido Electrónico\n
            Sistema de combustible	Carburado\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.40 x 2.36" / 61 x 60mm\n
            `,
                imagen: el1545,
                imagenDimension: "32%",
                pdf: fichaQuinceE
            },
            {

                id: "20 M/ ML 4S",
                nombre: "20 M/ ML 4S",
                caracteristicas: `hp / kW	20 / 14.7\n
            Tipo de motor	en línea 2\n
            Desplazamiento (CID/ CC)	21.4 / 351\n
            Empuje total (RPM)	5400 - 6100\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	115lbs / 52kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	CDI c/ Avance de Encendido Electrónico\n
            Sistema de combustible	Carburado\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.40 x 2.36" / 61 x 60mm\n
            `,
                imagen: ml20,
                imagenDimension: "32%",
                pdf: fichaVeinteM
            },
            {

                id: "40 ELPT 4S EFI",
                nombre: "40 ELPT 4S EFI",
                caracteristicas: `hp / kW	40 / 30\n
            Tipo de motor	en línea 3\n
            Desplazamiento (CID/ CC)	45.6 / 747\n
            Empuje total (RPM)	5500 - 6000\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	2.00:1\n
            Peso en seco *el modelo más ligero disponible	93kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	Módulo de control electrónico ECM 70\n
            Sistema de combustible	Inyección de combustible Electrónica\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.56 x 2.95" / 65 x 75mm\n
            `,
                imagen: elpt40,
                imagenDimension: "28%",
                pdf: fichaCuarentaEl
            },
            {

                id: "50 ELPT 4S EFI",
                nombre: "50 ELPT 4S EFI",
                caracteristicas: `hp / kW	50 / 37\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	60.8 / 995\n
            Empuje total (RPM)	5000 - 6000\n
            Longitud de pata	20" / 508mm\n
            Proporción de la caja de engranajes	1.83:1\n
            Peso en seco *el modelo más ligero disponible	112kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	Módulo de control electrónico ECM 70\n
            Sistema de combustible	Inyección de combustible Electrónica\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.56 x 2.95" / 65 x 75mm\n
            `,
                imagen: elpt50,
                imagenDimension: "28%",
                pdf: fichaCincuentaElpt
            },
            {

                id: "60 ELPT 4S EFI",
                nombre: "60 ELPT 4S EFI",
                caracteristicas: `hp / kW	60 / 45\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	60.8 / 995\n
            Empuje total (RPM)	5500 - 6000\n
            Longitud de pata	20" / 508mm\n
            Proporción de la caja de engranajes	1.83:1\n
            Peso en seco *el modelo más ligero disponible	112kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	Módulo de control electrónico ECM 70\n
            Sistema de combustible	Inyección de combustible Electrónica\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.56 x 2.95" / 65 x 75mm
            `,
                imagen: elpt60,
                imagenDimension: "28%",
                pdf: fichaSesenta
            },
            {

                id: "60 ELPT 4S EFI CT",
                nombre: "60 ELPT 4S EFI CT",
                caracteristicas: `hp / kW	60 / 45\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	60.8 / 995\n
            Empuje total (RPM)	5500 - 6000\n
            Longitud de pata	25" / 635mm\n
            Proporción de la caja de engranajes	2.33:1\n
            Peso en seco *el modelo más ligero disponible	118kg\n
            Clasificación de estrellas CARB	3\n
            Ignición	ECM 07 Digital Inductive\n
            Sistema de combustible	Inyección de combustible Electrónica\n
            Sistema de escape	A través de hélice\n
            Diámetro y carrera	2.56 x 2.95" / 65 x 75mm\n
            `,
                imagen: elpt60,
                imagenDimension: "28%",
                pdf: fichaSesentaCt
            }

        ],
        [
            {

                id: "75 ELPT 4S EFI",
                nombre: "75 ELPT 4S EFI",
                caracteristicas: `hp / kW	75 / 56\n
                    Tipo de motor	en línea 4\n
                    Desplazamiento (CID/ CC)	2.1 L\n
                    Empuje total (RPM)	4500 - 5500\n
                    Longitud de pata	20" / 508mm\n
                    Proporción de la caja de engranajes	2.07:1\n
                    Peso en seco *el modelo más ligero disponible	163kg\n
                    Clasificación de estrellas CARB	3\n
                    Ignición	SmartCraft ECM 70 Inductivo Digital\n
                    Sistema de combustible	Inyección de combustible Electrónica\n
                    Sistema de escape	A través de hélice\n
                    Diámetro y carrera	3.5 x 3.2" / 90 x 81mm\n
        `,
                imagen: cuatroTempos75elpt,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_Setenta,
                slides: 7
            },
            {

                id: "90 ELPT 4S EFI",
                nombre: "90 ELPT 4S EFI",
                caracteristicas: `hp / kW	90 / 67\n
                    Tipo de motor	en línea 4\n
                    Desplazamiento (CID/ CC)	2.1 L\n
                    Empuje total (RPM)	5000 - 6000\n
                    Longitud de pata	20" / 508mm | 25" / 635mm\n
                    Proporción de la caja de engranajes	2.07:1\n
                    Peso en seco *el modelo más ligero disponible	163kg\n
                    Clasificación de estrellas CARB	3\n
                    Ignición	SmartCraft ECM 70 Inductivo Digital\n
                    Sistema de combustible	Inyección de combustible Electrónica\n
                    Sistema de escape	A través de hélice\n
                    Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
        `,
                imagen: cuatroTempos90elptefi,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_Noventaelpt
            },
            {

                id: "115 ELPT 4S EFI",
                nombre: "115 ELPT 4S EFI",
                caracteristicas: `hp / kW	115 / 86\n
                Tipo de motor	en línea 4 | Árbol de levas individual de 8 válvulas (SOHC)\n
                Desplazamiento (CID/ CC)	2.1\n
                Empuje total (RPM)	5000 - 6000\n
                Longitud de pata	20" / 508mm | 25" / 635mm\n
                Proporción de la caja de engranajes	2.07:1\n
                Peso en seco *el modelo más ligero disponible	359lbs / 163kg\n
                Clasificación de estrellas CARB	3\n
                Ignición	SmartCraft ECM 70 Inductivo Digital\n
                Sistema de combustible	Inyección de combustible Electrónica\n
                Sistema de escape	A través de hélice\n
                Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
        `,
                imagen: cuatroTempos115elpt,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_CientoQuince_elpt_s
            },
            {

                id: "115 ELPT 4S EFI CT",
                nombre: "115 ELPT 4S EFI CT",
                caracteristicas: `hp / kW	115 / 86\n
                Tipo de motor	en línea 4 | Árbol de levas individual de 8 válvulas (SOHC)\n
                Desplazamiento (CID/ CC)	2.1\n
                Empuje total (RPM)	5000 - 6000\n
                Longitud de pata	20" / 508mm | 25" / 635mm\n
                Proporción de la caja de engranajes	2.07:1\n
                Peso en seco *el modelo más ligero disponible	359lbs / 163kg\n
                Clasificación de estrellas CARB	3\n
                Ignición	SmartCraft ECM 70 Inductivo Digital\n
                Sistema de combustible	Inyección de combustible Electrónica\n
                Sistema de escape	A través de hélice\n
                Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
        `,
                imagen: cuatroTempos115elptct,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_CientoQuinceCt
            },
            {

                id: "115 ELPT 4S EFI Pro XS",
                nombre: "115 ELPT 4S EFI Pro XS",
                caracteristicas: `hp / kW	115 / 86
                Tipo de motor	en línea 4
                Desplazamiento (CID/ CC)	2.1 L
                Empuje total (RPM)	5300 - 6300
                Longitud de pata	20" / 508mm | 25" / 635mm
                Proporción de la caja de engranajes	2.07:1
                Peso en seco *el modelo más ligero disponible	163kg
                Ignición	SmartCraft ECM 70 Inductivo Digital
                Sistema de combustible	Inyección de combustible Electrónica
                Sistema de escape	A través de hélice
                Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
        `,
                imagen: cuatroTempos115elptpro,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_CientoQuince_efiproxs
            },
            {

                id: "150 L/XL 4S EFI",
                nombre: "150 L/XL 4S EFI",
                caracteristicas: `hp / kW	150 / 110
                Tipo de motor	en línea 4
                Desplazamiento (CID/ CC)	3.0 L
                Empuje total (RPM)	5000 - 5800
                Longitud de pata	20" / 508mm | 25" / 635mm
                Proporción de la caja de engranajes	1.92:1
                Peso en seco *el modelo más ligero disponible	206kg
                Clasificación de estrellas CARB	3
                Ignición	SmartCraft ECM 70 Inductivo Digital
                Sistema de combustible	Inyección de combustible Electrónica
                Sistema de escape	A través de hélice
                Diámetro y carrera	4.0 x 3.6" / 102 x 92mm
        `,
                imagen: cuatroTempos150xl,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_CientoCincuenta_lx_efi
            },
            {

                id: "150 L/ XL 4S EFI Pro XS",
                nombre: "150 L/ XL 4S EFI Pro XS",
                caracteristicas: `hp / kW	150 / 110
                Tipo de motor	en línea 4
                Desplazamiento (CID/ CC)	3.0 L
                Empuje total (RPM)	5200 - 6000
                Longitud de pata	20" / 508mm | 25" / 635mm
                Proporción de la caja de engranajes	2.08:1
                Peso en seco *el modelo más ligero disponible	207kg
                Clasificación de estrellas CARB	3
                Ignición	SmartCraft ECM 70 Inductivo Digital
                Sistema de combustible	Inyección de combustible Electrónica
                Sistema de escape	A través de hélice
                Diámetro y carrera	4.0 x 3.6" / 102 x 92mm
        `,
                imagen: cuatroTempos150xl4sefipro,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_CientoCincuenta_xl_efi_pro
            },
        ],
        [
            {

                id: "200 L/XL 4S EFI",
                nombre: "200 L/XL 4S EFI",
                caracteristicas: `hp / kW	200 / 149
                Tipo de motor	V6
                Desplazamiento (CID/ CC)	209 CID/ 3.4 L
                Empuje total (RPM)	5000 - 5800
                Longitud de pata	20" / 508mm | 25" / 635mm
                Proporción de la caja de engranajes	1.85:1
                Peso en seco *el modelo más ligero disponible	216kg
                Configuración del cilindro	24-valve DOHC
                Ampers / Watts del alternador	85 amp (1071 watt)
                Encendido	SmartStart Electric
                Sistema de corte	Power Trim and Tilt
                Dirección	Hidráulico
        `,
                imagen: cuatroTempos200,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_Doscientos,
                slides: 3
            },
            {

                id: "250 L/XL 4S EFI",
                nombre: "250 L/XL 4S EFI",
                caracteristicas: `hp / kW	250 / 186
                Tipo de motor	V8
                Desplazamiento (CID/ CC)	279 CID/ 4.6 L
                Empuje total (RPM)	5200 - 6000
                Longitud de pata	20" / 508mm | 25" / 635mm | 30" / 762mm
                Proporción de la caja de engranajes	1.75:1
                Peso en seco *el modelo más ligero disponible	239kg
                Configuración del cilindro	32-valve DOHC
                Ampers / Watts del alternador	115 amp (1449 watt)
                Encendido	SmartStart Electric
                Sistema de corte	Power Trim and Tilt
                Dirección	Hidráulico
        `,
                imagen: cuatroTempos250,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_DoscientosCincuenta
            },
            {

                id: "300 L/XL 4S EFI",
                nombre: "300 L/XL 4S EFI",
                caracteristicas: `hp / kW	300 / 224
                Tipo de motor	V8
                Desplazamiento (CID/ CC)	279 CID/ 4.6 L
                Empuje total (RPM)	5200 - 6000
                Longitud de pata	20" / 508mm | 25" / 635mm
                Proporción de la caja de engranajes	1.75:1
                Peso en seco *el modelo más ligero disponible   239kg
                Configuración del cilindro	32-valve DOHC
                Ampers / Watts del alternador		85 amp (1071 watt)
                Encendido	SmartStart Electric
                Sistema de corte	Power Trim and Tilt
                Dirección	Hidráulico
        `,
                imagen: cuatroTempos300,
                imagenDimension: "28%",
                pdf: ficha_cuatroTiempos_Tresientos
            }
        ]
    ],
    [
        {
            name: "3.5-60 Hp",
            id: "uno"
        },
        {
            name: "75-150 Hp",
            id: "dos"
        },
        {
            name: "200-300 hp",
            id: "tres"
        }
    ]
]
// -------------------------------------------------------SEA PRO-------------------------------------------------
export const seaProInfo = [
    // [

    //     {
    //         id: "60 ELPT 4S SP",
    //         nombre: "60 ELPT 4S SP",
    //         caracteristicas: `hp / kW: 60 / 45\n
    //     Tipo de motor: en línea 4\n
    //     Desplazamiento (CID/ CC): 	61 / 995\n
    //     Empuje total (RPM): 4500 - 5500\n
    //     Longitud de pata: 20" / 508mm | 22.5" 572mm\n
    //     Proporción de la caja de engranajes: 2.33:1`,
    //         imagen: seapro2
    //         ,
    //         imagenDimension: "50%",
    //         slides: 2.9,
    //         pdf: ficha_seapro_sesenta_elpcuatro
    //     },
    //     {
    //         id: "60 ELPT 4S BT SP",
    //         nombre: "60 ELPT 4S BT SP",
    //         caracteristicas: `hp / kW: 60 / 45\n
    //     Tipo de motor: en línea 4\n
    //     Desplazamiento (CID/ CC): 	61 / 995\n
    //     Empuje total (RPM): 4500 - 5500\n
    //     Longitud de pata: 20" / 508mm | 22.5" 572mm\n
    //     Proporción de la caja de engranajes: 2.33:1\n
    //     `,
    //         imagen: seapro1,
    //         imagenDimension: "50%",
    //         pdf: ficha_seapro_sesenta_elptcuatros
    //     },
    //     {
    //         id: "75 ELPT 4S SP",
    //         nombre: "75 ELPT 4S SP",
    //         caracteristicas: `hp / kW: 75 / 56\n
    //     Tipo de motor: en línea 4\n
    //     Desplazamiento (CID/ CC): 	2.1 L\n
    //     Empuje total (RPM): 5000 - 5500\n
    //     Longitud de pata: 20" / 508mm\n
    //     Proporción de la caja de engranajes: 2.38:1
    //     `,
    //         imagen: seapro,
    //         imagenDimension: "50%",
    //         pdf: ficha_seapro_setentaCinco
    //     },
    //     {
    //         id: "75 ELPT 4S SP",
    //         nombre: "75 ELPT 4S SP",
    //         caracteristicas: `hp / kW: 75 / 56\n
    //     Tipo de motor: en línea 4\n
    //     Desplazamiento (CID/ CC): 	2.1 L\n
    //     Empuje total (RPM): 5000 - 5500\n
    //     Longitud de pata: 20" / 508mm\n
    //     Proporción de la caja de engranajes: 2.38:1
    //     `,
    //         imagen: seapro,
    //         imagenDimension: "50%",
    //         pdf: ficha_seapro_setentaCincoCopia
    //     },
    // ],
    [
        // --------------------------------------------------------ACAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA------------------------------------------------
        [{

            id: "60 ELPT 4S SP",
            nombre: "60 ELPT 4S SP",
            caracteristicas: `hp / kW	60 / 45\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	61 / 995\n
            Empuje total (RPM)	4500 - 5500\n
            Longitud de pata	20" / 508mm | 22.5" 572mm | 25" / 635mm\n
            Proporción de la caja de engranajes	2.33:1\n
            Peso en seco *el modelo más ligero disponible	118kg\n
            Ignición	SmartCraft ECM 07 Inductivo Digital\n
            Sistema de escape	A través de hélice\n
            Inducción de aire	Colector de admisión mejorado para desempeño\n
            Ampers / Watts del alternador	18 / 226\n
            Diámetro y carrera	2.56 x 2.95" / 65 x 75mm
            `,
            imagen: seapro60elpt,
            imagenDimension: "28%",
            pdf: ficha_seapro_sesenta_elpcuatro,
            slides: 8

        },
        {

            id: "60 ELPT 4S BT SP",
            nombre: "60 ELPT 4S BT SP",
            caracteristicas: `
            hp / kW	60 / 45
            Tipo de motor	en línea 4
            Desplazamiento (CID/ CC)	61 / 995
            Empuje total (RPM)	4500 - 5500
            Longitud de pata	20" / 508mm | 22.5" 572mm | 25" / 635mm
            Proporción de la caja de engranajes	2.33:1
            Peso en seco *el modelo más ligero disponible	118kg
            Ignición	SmartCraft ECM 07 Inductivo Digital
            Sistema de escape	A través de hélice
            Inducción de aire	Colector de admisión mejorado para desempeño
            Ampers / Watts del alternador	18 / 226
            Diámetro y carrera	2.56 x 2.95" / 65 x 75mm
            `,
            imagen: seapro60elpt,
            imagenDimension: "28%",
            pdf: ficha_seapro_sesenta_elptcuatros

        },
        {

            id: "75 ELHPT 4S SP",
            nombre: "75 ELHPT 4S SP",
            caracteristicas: `
            hp / kW	75 / 56\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	2.1 L\n
            Empuje total (RPM)	5000 - 5500\n
            Longitud de pata	20" / 508mm\n
            Proporción de la caja de engranajes	2.38:1\n
            Peso en seco *el modelo más ligero disponible	165kg\n
            Ignición	SmartCraft ECM 70 Inductivo Digital\n
            Sistema de escape	A través de hélice\n
            Ampers / Watts del alternador	35 / 441 con regulador de voltaje enfriado por agua\n
            Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
            `,
            imagen: seapro75,
            imagenDimension: "28%",
            pdf: ficha_seapro_setentaCinco

        },
        {

            id: "90 ELPT 4S SP",
            nombre: "90 ELPT 4S SP",
            caracteristicas: `
            hp / kW	90 / 67\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	2.1 L\n
            Empuje total (RPM)	5000 - 5500\n
            Longitud de pata	20" / 508mm | 25" / 635mm\n
            Proporción de la caja de engranajes	2.38:1\n
            Peso en seco *el modelo más ligero disponible	165kg\n
            Ignición	SmartCraft ECM 70 Inductivo Digital\n
            Sistema de escape	A través de hélice\n
            Ampers / Watts del alternador	35 / 441 con regulador de voltaje enfriado por agua\n
            Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
            `,
            imagen: seapro90,
            imagenDimension: "28%",
            pdf: ficha_seapro_Noventa_elpt

        },
        {

            id: "90 ELPT 4S BT SP",
            nombre: "90 ELPT 4S BT SP",
            caracteristicas: `
            hp / kW	90 / 67\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	2.1 L\n
            Empuje total (RPM)	5000 - 5500\n
            Longitud de pata	20" / 508mm | 25" / 635mm\n
            Proporción de la caja de engranajes	2.38:1\n
            Peso en seco *el modelo más ligero disponible	165kg\n
            Ignición	SmartCraft ECM 70 Inductivo Digital\n
            Sistema de escape	A través de hélice\n
            Ampers / Watts del alternador	35 / 441 con regulador de voltaje enfriado por agua\n
            Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
            `,
            imagen: seapro90,
            imagenDimension: "28%",
            pdf: ficha_seapro_NoventaElpt

        },
        {

            id: "115 ELPT 4S SP",
            nombre: "115 ELPT 4S SP",
            caracteristicas: `
            hp / kW	115 / 86\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	2.1 L\n
            Empuje total (RPM)	5000 - 5500\n
            Longitud de pata	20" / 508mm | 25" / 635mm\n
            Proporción de la caja de engranajes	2.38:1\n
            Peso en seco *el modelo más ligero disponible	165kg\n
            Ignición	SmartCraft ECM 70 Inductivo Digital\n
            Sistema de escape	A través de hélice\n
            Ampers / Watts del alternador	35 / 441 con regulador de voltaje enfriado por agua\n
            Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
            `,
            imagen: seapro115,
            imagenDimension: "28%",
            pdf: ficha_seapro_ciento_quince

        },
        {

            id: "115 ELPT 4S BT SP",
            nombre: "115 ELPT 4S BT SP",
            caracteristicas: `
            hp / kW	115 / 86
            Tipo de motor	en línea 4
            Desplazamiento (CID/ CC)	2.1 L
            Empuje total (RPM)	5000 - 5500
            Longitud de pata	20" / 508mm | 25" / 635mm
            Proporción de la caja de engranajes	2.38:1
            Peso en seco *el modelo más ligero disponible	165kg
            Ignición	SmartCraft ECM 70 Inductivo Digital
            Sistema de escape	A través de hélice
            Ampers / Watts del alternador	35 / 441 con regulador de voltaje enfriado por agua
            Diámetro y carrera	3.5 x 3.2" / 90 x 81mm
            `,
            imagen: seapro115,
            imagenDimension: "28%",
            pdf: ficha_seapro_cientoQuince_elpt

        },
        {

            id: "150 L/XL 4S SP",
            nombre: "150 L/XL 4S SP",
            caracteristicas: `
            hp / kW	150 / 110\n
            Tipo de motor	en línea 4\n
            Desplazamiento (CID/ CC)	3.0 L\n
            Empuje total (RPM)	4800 - 5300\n
            Longitud de pata	20" / 508mm | 25" / 635mm\n
            Proporción de la caja de engranajes	1.92:1\n
            Peso en seco *el modelo más ligero disponible	206kg\n
            Ignición	SmartCraft ECM 70 Inductivo Digital\n
            Sistema de escape	A través de hélice\n
            Ampers / Watts del alternador	60 / 756 (accionado por correa)\n
            Diámetro y carrera	4.0 x 3.6" / 102 x 92mm\n
            `,
            imagen: seapro150sp,
            imagenDimension: "28%",
            pdf: ficha_seapro_ciento_cincuenta

        },
        ],
        [
            {

                id: "200 L/XL 4S SP",
                nombre: "200 L/XL 4S SP",
                caracteristicas: `
            hp / kW	200 / 149\n
            Tipo de motor	V6\n
            Desplazamiento (CID/ CC)	209 CID/ 3.4 L\n
            Empuje total (RPM)	4600 - 5400\n
            Longitud de pata	20" / 508mm | 25" / 635mm\n
            Proporción de la caja de engranajes	1.85:1\n
            Peso en seco *el modelo más ligero disponible	220kg\n
            Configuración del cilindro	24-valve DOHC\n
            Ampers / Watts del alternador	85 / 1071\n
            Encendido	SmartStart Electric\n
            Sistema de corte	Power Trim and Tilt\n
            Dirección	Hydraulic/ Electro-Hydraulic
            `,
                imagen: seapro200,
                imagenDimension: "28%",
                pdf: ficha_seapro_doscientos_unico,
                slides: 1

            },
        ], [
            {

                id: "500 HP",
                nombre: "500 HP",
                caracteristicas: `
            hp / kW	500\n
            Tipo de motor	V12\n
            Desplazamiento (CID/ CC)	461\n
            Empuje total (RPM)	5000-5800\n
            Longitud de pata	20" / 508 mm | 25" / 635 mm | 30" / 762 mm | 35" / 889mm\n
            Proporción de la caja de engranajes	2.50:1\n
            Peso en seco *el modelo más ligero disponible	1260lbs / 572kg\n
            Configuración del cilindro	V12\n
            Ampers / Watts del alternador	150 amp with Idle Charge\n
            Encendido	Electric Start with SmartStart Protection\n
            Sistema de corte	Power Trim and Tilt\n
            Dirección	Integrated Electro-Hydraulic Power-Steering\n
            Desplazamiento (L)	7.6L
            `,
                imagen: seapro500,
                imagenDimension: "28%",
                pdf: ficha_seapro_quinientos_unico,
                slides: 1

            },
        ]
    ],
    [
        {
            name: "60-150 Hp",
            id: "uno"
        },
        {
            name: "200 Hp",
            id: "dos"
        },
        {
            name: "500 Hp",
            id: "tres"
        }
    ]

]

// -------------------------------VERADO--------------------------------------
export const veradoInfo = [
    // [
    //     {
    //         id: "250-300 Hp",
    //         nombre: "250-300 Hp",
    //         caracteristicas: `hp / kW: 250 Hp\n
    //         Tipo de motor: V8\n
    //         Desplazamiento (CID/ CC): 279 CID/ 4.6 L\n
    //         Empuje total (RPM): 5200 - 6000\n
    //         Longitud de pata:20" /508mm |25" /635mm  30" /762mm\n
    //         Proporción de la caja de engranajes:	1.85:1`,
    //         imagen: verado2
    //         ,
    //         imagenDimension: "50%",
    //         slides: 2.9
    //     },
    //     {
    //         id: "350-400 Hp",
    //         nombre: "350-400 Hp",
    //         caracteristicas: `hp / kW: 350\n
    //         Empuje total RPM: 5300-5900\n
    //         Tipo de motor: V10\n
    //         Amperaje: 150 amp\n
    //         Longitud de Pata: 20" / 508 mm | 25" / 635 mm |  30" / 762 mm | 35" / 889mm
    //     `,
    //         imagen: verado2,
    //         imagenDimension: "50%",
    //     },
    //     {
    //         id: "600 Hp",
    //         nombre: "600 HP",
    //         caracteristicas: `hp / kW: 600'n
    //         Tipo de motor: V12'n
    //         Desplazamiento (CID/ CC): 461'n
    //         Empuje total (RPM): 5600-6400'n
    //         Longitud de pata: 20" / 508 mm | 25" / 635 mm | 30" / 762 mm | 35" / 889mm
    //     `,
    //         imagen: verado2,
    //         imagenDimension: "50%"
    //     },
    // ],
    [
        [

            {
                id: "250 Hp",
                nombre: "VERADO 250 Hp",
                caracteristicas: `hp / kW: 250 Hp\n
            Tipo de motor: V8\n
            Desplazamiento (CID/ CC): 279 CID/ 4.6 L\n
            Empuje total (RPM): 5200 - 6000\n
            Longitud de pata: 20" / 508mm | 25" / 635mm | 30" / 762mm\n
            Proporción de la caja de engranajes:	1.85:1\n
            Peso en seco *el modelo más ligero disponible: 272kg\n
            Configuración del cilindro: 32-valve DOHC\n
            Ampers / Watts del alternador: 115 / 1449\n
            Encendido: SmartStart Electric	\n
            SmartStart Electric: Power Trim and Tilt
        `,
                imagen: verado250hp,
                imagenDimension: "28%",
                pdf: ficha_verado_doscincuenta,
                slides: 2
            },
            {
                id: "300 Hp",
                nombre: "VERADO 300 Hp",
                caracteristicas: `hp / kW  300 / 224\n
            Tipo de motor V8\n
            Desplazamiento (CID/ CC)	279 CID/ 4.6 L\n
            Empuje total (RPM)	5200 - 6000\n
            Longitud de pata   20" / 508mm | 25" / 635mm | 30" / 762mm\n
            Proporción de la caja de engranajes	1.75:1 / 1.85:1\n
            Peso en seco *el modelo más ligero disponible	272kg\n
            Configuración del cilindro	24-valve DOHC\n
            Ampers / Watts del alternador   115 / 1449\n
            Encendido	SmartStart Electric\n
            Sistema de corte	Power Trim and Tilt\n
            Dirección	Integrated Electro-Hydraulic
        `,
                imagen: verado300hp,
                imagenDimension: "28%",
                pdf: ficha_verado_trescientos
            },
        ],

        [
            {
                id: "350 hp",
                nombre: "Verado 350 hp",
                caracteristicas: `hp / kW: 350\n
            Empuje total RPM: 5300-5900\n
            Tipo de motor: V10\n
            Cilindrada: 5,7L (349 CID)\n
            Arranque: Eléctrico con protección SmartStart\n
            Relación de caja de engranaje: 2.08:1\n
            Amperaje: 150 amp\n
            Sistema de corte: Power trim y tilt\n
            Peso en seco: 316 kg (695 lbs)\n
            Dirección: Dirección electrohidráulica\n
            Longitud de Pata: 20" / 508 mm | 25" / 635 mm |  30" / 762 mm | 35" / 889mm
        `,
                imagen: verado10_port_b_350,
                imagenDimension: "28%",
                pdf: ficha_verado_tresCincuenta_and_cuatrocientos,
                slides: 2
            },

            {
                id: "400 hp",
                nombre: "Verado 400 hp",
                caracteristicas: `hp / kW: 400\n
            Empuje total RPM: 5300-5900\n
            Tipo de motor: V10\n
            Cilindrada: 5,7L (349 CID)\n
            Inducción: Colector de admisión mejorado de aspiración natural\n
            Arranque: Eléctrico con protección SmartStart\n
            Relación de caja de engranaje: 2.08:1\n
            Amperaje: 150 amp\n
            Sistema de corte: Power trim y tilt\n
            Peso en seco: 316 kg (695 lbs)\n
            Dirección: Dirección electrohidráulica\n
            Longitud eje xxl / cxxl: 762 mm (30 in)\n
        `,
                imagen: verado10_port_b_400,
                imagenDimension: "28%",
                pdf: ficha_verado_tresCincuenta_and_cuatrocientos
            },

        ],

        [
            {
                id: "Verado 600hp",
                nombre: "Verado 600hp",
                caracteristicas: `hp / kW	600\n
            Tipo de motor	V12\n
            Desplazamiento (CID/ CC)	461\n
            Empuje total (RPM)	5600-6400\n
            Longitud de pata	20" / 508 mm | 25" / 635 mm | 30" / 762 mm | 35" / 889mm\n
            Proporción de la caja de engranajes	2.50:1\n
            Peso en seco *el modelo más ligero disponible	1260lbs / 572kg\n
            Configuración del cilindro	V12\n
            Ampers / Watts del alternador	150 amp\n
            Encendido	Electric Start with SmartStart Protection\n
            Sistema de corte	Power Trim and Tilt\n
            Dirección	Integrated Electro-Hydraulic Power-Steering\n
            Desplazamiento (L)	7.6L
        `,
                imagen: veradoV12_600,
                imagenDimension: "28%",
                pdf: ficha_verado_seiscientos,
                slides: 1
            },
        ]
    ],
    [
        { name: "250-300 Hp", id: "uno" },
        { name: "350-400 Hp", id: "dos" },
        { name: "600 Hp", id: "tres" }

    ]
]

// --------------------------------DOS TEMPOS-------------------
export const dosTemposInfo = [
    // [
    //     {
    //         id: "3.3 M",
    //         nombre: "3.3 M",
    //         caracteristicas: `hp / kW: 3.3 / 2.5\n
    //     Encendido: Manual\n
    //     Configuración del cilindro: 	1\n
    //     Empuje total (RPM): 4500 - 5500\n
    //     Longitud de pata: 15" / 381mm\n
    //     Proporción de la caja de engranajes: 2.15:1`,
    //         imagen: dosTempos33m,
    //         imagenDimension: "50%",
    //         slides: 2,
    //         pdf: ficha_dosTiempos_tres_tres
    //     },
    //     {
    //         id: "5M/ ML",
    //         nombre: "5M/ ML",
    //         caracteristicas: `hp / kW: 5/3.7\n
    //     Encendido: Manual\n
    //     Configuración del cilindro: 	1\n
    //     Empuje total (RPM): 4000 - 5000\n
    //     Longitud de pata: 15" / 381mm | 20" / 508mm\n
    //     Proporción de la caja de engranajes: 2.15:1`,
    //         imagen: dosTempos2,
    //         imagenDimension: "50%",
    //         pdf: ficha_dosTiempos_cinco
    //     },
    // ],
    [
        [{
            id: "3.3 M",
            nombre: "3.3 M",
            caracteristicas: `hp / kW	3.3 / 2.5\n
            Empuje total (RPM)	4500 - 5500\n
            Longitud de pata	15" / 381mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	13kg\n
            Configuración del cilindro	1\n
            Diámetro y carrera	47 x 43\n
            Inyección de Aceite	pre-mezcla\n
            Encendido	manual`,
            imagen: dosTempos33m,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_tres_tres,
            slides: 9

        },
        {
            id: "5 M/ML",
            nombre: "5 M/ML",
            caracteristicas: `hp / kW	5/3.7\n
            Empuje total (RPM)	4000 - 5000\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	2.15:1\n
            Peso en seco *el modelo más ligero disponible	20kg\n
            Configuración del cilindro	1\n
            Diámetro y carrera	55 x 43\n
            Inyección de Aceite	pre-mezcla\n
            Encendido	manual`,
            imagen: dosTempos5mml,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_cinco
        },
        {
            id: "15 M Super",
            nombre: "15 M Super",
            caracteristicas: `hp / kW	15 / 11.4\n
            Empuje total (RPM)	5200 - 5800\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	1.85:1\n
            Peso en seco *el modelo más ligero disponible	41kg\n
            Configuración del cilindro	L-2\n
            Diámetro y carrera	60m x 52mm\n
            Inyección de Aceite	pre-mezcla\n
            Encendido	manual`,
            imagen: dosTempos15mSuper,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_quince
        },
        {
            id: "25 M/ML Sea Pro",
            nombre: "25 M/ML Sea Pro",
            caracteristicas: `hp / kW	25 / 18.4\n
            Empuje total (RPM)	5000 - 6000\n
            Longitud de pata	15" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	1.92:1\n
            Peso en seco *el modelo más ligero disponible	51kg\n
            Configuración del cilindro	L-2\n
            Diámetro y carrera	68 x 59mm\n
            Inyección de Aceite	pre-mezcla\n
            Encendido	Eléctrico y Manual`,
            imagen: dosTempos25ml,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_veinticinco
        },
        {
            id: "30 M/ML Super",
            nombre: "30 M/ML Super",
            caracteristicas: `hp / kW	30 / 22.5\n
            Empuje total (RPM)	5150 - 5850\n
            Longitud de pata	51" / 381mm | 20" / 508mm\n
            Proporción de la caja de engranajes	1.92:1\n
            Peso en seco *el modelo más ligero disponible	51kg\n
            Configuración del cilindro	L-2\n
            Diámetro y carrera	68 x 59mm\n
            Inyección de Aceite	pre-mezcla\n
            Encendido	Eléctrico y Manual`,
            imagen: dosTempos30ml,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_treinta
        },
        {
            id: "40 M/ML Super",
            nombre: "40 M/ML Super",
            caracteristicas: `hp / kW	40 / 29.8\n
            Empuje total (RPM)	5000 - 5700\n
            Longitud de pata	20" / 508mm\n
            Proporción de la caja de engranajes	1.85:1\n
            Peso en seco *el modelo más ligero disponible	72kg\n
            Configuración del cilindro	L-3\n
            Diámetro y carrera	68 x 64 mm\n
            Inyección de Aceite	Estándar y pre-mezcla\n
            Encendido	Eléctrico y Manual`,
            imagen: dosTempos40ml,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_cuarenta_ml_super
        },
        {
            id: "40 EO/ ELO Super",
            nombre: "40 EO/ ELO Super",
            caracteristicas: `hp / kW	40 / 29.8\n
            Empuje total (RPM)	5000 - 5700\n
            Longitud de pata	20" / 508 mm\n
            Proporción de la caja de engranajes	1.85 : 1\n
            Peso en seco *el modelo más ligero disponible	72kg\n
            Configuración del cilindro	L-3\n
            Diámetro y carrera	68 x 64 mm\n
            Inyección de Aceite	Estándar\n
            Encendido	Eléctrico`,
            imagen: dosTempos40eoelo,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_cuarenta_elo_super
        },
        {
            id: "40 ELPTO",
            nombre: "40 ELPTO",
            caracteristicas: `hp / kW	40 / 29.8\n
            Empuje total (RPM)	5000 - 5700\n
            Longitud de pata	20" / 508mm\n
            Proporción de la caja de engranajes	1.85 : 1\n
            Peso en seco *el modelo más ligero disponible	72kg\n
            Configuración del cilindro	L-3\n
            Diámetro y carrera	68 x 64 mm\n
            Inyección de Aceite	Estándar\n
            Encendido	Eléctrico`,
            imagen: dosTempos40elpto,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_cuarenta_elpto
        },
        {
            id: "50 ELPTO",
            nombre: "50 ELPTO",
            caracteristicas: `hp / kW	50 / 37\n
            Empuje total (RPM)	5150 - 5850\n
            Longitud de pata	20" / 508mm\n
            Proporción de la caja de engranajes	1.85:1\n
            Peso en seco *el modelo más ligero disponible	85kg\n
            Desplazamiento (CID/ CC)	42.53/697\n
            Diámetro y carrera	68 x 64 mm\n
            Inyección de Aceite	Estándar\n
            Encendido	Eléctrico`,
            imagen: dosTempos50elpto,
            imagenDimension: "28%",
            pdf: ficha_dosTiempos_cincuenta_elpto
        },
        ],
    ],

    [
        { id: "uno", name: "3.3-90 Hp" }
    ]
]



export const racinInfo = [
    // [

    //     {
    //         id: "450R hp",
    //         nombre: "450R hp",
    //         caracteristicas: `hp / kW: 450\n
    //     Desplazamiento (CID/ CC): 279 CID / 4.6L\n
    //     Empuje total (RPM): 5800-6400\n
    //     Proporción de la caja de engranajes: 1.60:1\n
    //     Ignición: SmartCraft PCM 112 Digital Inductive\n
    //     Encendido: SmartStart Electric`,
    //         imagen: racing,
    //         imagenDimension: "50%",
    //         slides: 1,
    //         pdf: ficha_racing_cuatrocientoscincuenta
    //     }
    // ],
    [
        [{
            id: "450R hp",
            nombre: "450R hp",
            caracteristicas: `hp / kW	450\n
        Desplazamiento (CID/ CC)	279 CID / 4.6L\n
        Empuje total (RPM)	5800-6400\n
        Proporción de la caja de engranajes	1.60:1\n
        Peso en seco *el modelo más ligero disponible	L-20": 689/313 (450R) | 700/318 (Sport Master) | XL-25": 702/318 (450R) | 714/324 (Sport Master) | XXL-30": 15/324 (450R) | 725/329 (Sport Master)\n
        Ignición	SmartCraft PCM 112 Digital Inductive\n
        Sistema de combustible	Electronic Fuel Injection (EFI) Computer Controlled\n
        Configuración del cilindro	V8 [64 degree] with 32-valve Dual Overhead Cam [DOHC]\n
        Diámetro y carrera	3.6 in x 3.4 in / 92 mm x 86 mm\n
        Encendido	SmartStart Electric\n
        Sistema de inducción	Supercharged with Charge Air Cooling and Electronic Boost Control\n
        Pata de transmisión	450R: 5.44” HD | 450R Sport Master`,
            imagen: racing,
            imagenDimension: "30%",
            pdf: ficha_racing_cuatrocientoscincuenta,
            slides: 1
        },
        ]
    ],
    [
        { name: "450R Hp", id: "uno" }
    ]
]